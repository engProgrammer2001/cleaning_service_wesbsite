...
const CivilWorkDetails = ({ CivilWordData }) => {
  const { id } = useParams();
  const service = CivilWordData.find((item) => item.id === parseInt(id));
        ...
        <div className="w-full lg:w-5/12 bg-gray-50 p-6 rounded-lg shadow-lg">
          ...
          <ul>
            {CivilWordData.map((item) => (
              ...
              >
                <Link to={`/civil-work-service/${item.id}`}>
...
export default CivilWorkDetails;
